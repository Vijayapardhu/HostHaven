...
import {
  ...
  updateReviewSchema,
  reviewFilterSchema,
  propertyReviewFilterSchema,
  vendorReviewFilterSchema,
...
export const ReviewsController = {
  ...
  async getPropertyReviews(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const query = propertyReviewFilterSchema.parse(request.query);
      ...
      return sendSuccess(
        reply,
        { reviews: result.reviews, stats: result.stats },
        200,
        result.meta,
      );
    } catch (error: any) {
  ...
  async getVendorReviews(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const query = vendorReviewFilterSchema.parse(request.query);

      const result = await reviewsService.getVendorReviews(vendorId, query);
  ...
  async respondToReview(request: FastifyRequest, reply: FastifyReply) {
    try {
      const { id } = reviewIdSchema.parse(request.params);
      const vendorId = (request as any).user.vendorId;
      const data = reviewResponseSchema.parse(request.body);
      ...
      if (error.name === 'ZodError') {
        return sendError(reply, ERROR_CODES.VALIDATION_ERROR, 'Invalid response text', 400);
      }
      return sendError(
        reply,
        ERROR_CODES.INTERNAL_ERROR,
  ...
  async checkEligibility(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const result = await reviewsService.checkEligibility(userId, propertyId);

      return sendSuccess(reply, result);
    } catch (error: any) {
      logger.error({ error }, "Check eligibility failed");
      if (error.code === ERROR_CODES.RESOURCE_NOT_FOUND) {
        return sendError(reply, error.code, error.message, 404);
      }
      return sendError(
