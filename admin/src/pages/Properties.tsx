...
import { Ban, CheckCircle, Eye, Star } from 'lucide-react'
...
const statusVariants: Record<PropertyStatus, 'success' | 'warning' | 'danger' | 'neutral'> = {
  approved: 'success',
  pending: 'warning',
  rejected: 'danger',
  inactive: 'neutral',
...
export default function Properties() {
  ...
  const handleStatusChange = (propertyId: string, nextStatus: PropertyStatus) => {
  ...
  const confirmStatusChange = async () => {
    ...
    try {
      if (nextStatus === 'approved') {
        ...
        await propertiesService.updateProperty(propertyId, { status: nextStatus })
      }
      setProperties((prev) => prev.map((property) => (property.id === propertyId ? { ...property, status: nextStatus } : property)))
  ...
  return (
    <div className="space-y-6">
          ...
          <TableBody>
            {properties.map((property) => (
              <TableRow key={property.id}>
                ...
                <TableCell>
                  <StatusBadge label={statusLabels[property.status]} variant={statusVariants[property.status]} />
                </TableCell>
                <TableCell>
                  <div className="flex items-center gap-1 text-sm text-slate-700">
                    <Star className="h-4 w-4 text-amber-500" />
                    {property.rating ?? 0}
                    <span className="text-xs text-slate-400">({property.reviewCount ?? 0})</span>
                  </div>
                </TableCell>
                <TableCell>
      ...
      <ConfirmDialog
        ...
        confirmText={confirmAction?.nextStatus === 'inactive' ? 'Deactivate property' : 'Activate property'}
        variant={confirmAction?.nextStatus === 'inactive' ? 'danger' : 'default'}
