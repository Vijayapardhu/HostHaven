...
export function PWAInstallPrompt() {
    ...
    useEffect(() => {
        ...
        if (localStorage.getItem("pwa-admin-dismissed") === "true") return;
    ...
    const handleDismiss = () => {
        setShowBanner(false);
        localStorage.setItem("pwa-admin-dismissed", "true");
    };
                    ...
                    <div className="bg-[#111118] border border-white/10 rounded-2xl p-4 shadow-2xl shadow-black/60 backdrop-blur-xl flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-white font-semibold text-sm leading-tight">
                                {isIOS ? "Add to Home Screen" : "Install Admin App"}
                            </p>
                            {isIOS ? (
                                <p className="text-white/50 text-xs mt-1 leading-relaxed">
                                    Tap <span className="text-blue-400 font-medium">Share ↑</span> then{" "}
                                    <span className="text-blue-400 font-medium">"Add to Home Screen"</span>
                                </p>
                            ) : (
                                <p className="text-white/50 text-xs mt-1 leading-relaxed">
                                    Access the admin panel offline on your device
                                </p>
                            )}
                            ...
                            {!isIOS && (
                                <button
                                    ...
                                    className="mt-2.5 inline-flex items-center gap-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-all active:scale-95"
                                >
