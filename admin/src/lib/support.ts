...
export const supportService = {
  ...
  updateTicketPriority: async (ticketId: string, priority: 'low' | 'medium' | 'high') => {
    throw new Error('Ticket priority update is not available on the backend')
  },

  addNote: async (ticketId: string, data: CreateTicketNoteRequest) => {
    throw new Error('Ticket notes endpoint is not available on the backend')
  },
  ...
  getCategories: async () => {
    throw new Error('Support categories endpoint is not available on the backend')
  },

  exportTickets: async (params?: Record<string, any>) => {
    throw new Error('Export support tickets endpoint is not available on the backend')
  },
}
